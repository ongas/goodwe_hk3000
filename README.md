# GWHK3000 Smart Meter — Home Assistant Integration

Local integration for the **GoodWe GWHK3000** smart meter dongle (HK1000 device).
...
The GWHK3000 speaks GoodWe's binary POSTGW protocol over TCP. This integration:

1. Acts as a TCP proxy listening on a configurable port (default `20001`)
2. The GWHK3000 must be pointed at your HA host's IP on that port (via the HF-A21 web UI at `http://<device-ip>/pages/app_config.html`)
...
Add this repository as a custom HACS repository (type: Integration), then install **GWHK3000 Smart Meter**.

## Configuration

1. In HA go to **Settings → Devices & Services → Add Integration → GWHK3000 Smart Meter**
2. Set:
...
3. Point the GWHK3000 device at your HA host IP on that port via its web UI

## Supported devices

- GoodWe GWHK3000 / HK1000 (serial prefix `93000HKU`)
